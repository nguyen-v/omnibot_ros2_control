...
struct Axis {
    Axis(SocketCanIntf* can_intf, uint32_t node_id) : can_intf_(can_intf), node_id_(node_id), reduction_ratio_(1.0) {}
...
CallbackReturn ODriveHardwareInterface::on_init(const hardware_interface::HardwareInfo& info) {
    ...
    for (auto& joint : info_.joints) {
        ...
        // Parse the reduction_ratio parameter; if missing, you could default to 1.0.
...
return_type ODriveHardwareInterface::write(const rclcpp::Time&, const rclcpp::Duration&) {
    for (auto& axis : axes_) {
        ...
        if (axis.pos_input_enabled_) {
            ...
            msg.Input_Pos = (axis.pos_setpoint_ * axis.reduction_ratio_) / (2 * M_PI);
            msg.Vel_FF = axis.vel_input_enabled_ ? ((axis.vel_setpoint_ * axis.reduction_ratio_) / (2 * M_PI)) : 0.0f;
            msg.Torque_FF = axis.torque_input_enabled_ ? (axis.torque_setpoint_ / axis.reduction_ratio_) : 0.0f;
            axis.send(msg);
        } else if (axis.vel_input_enabled_) {
            Set_Input_Vel_msg_t msg;
            msg.Input_Vel = (axis.vel_setpoint_ * axis.reduction_ratio_) / (2 * M_PI);
            msg.Input_Torque_FF = axis.torque_input_enabled_ ? (axis.torque_setpoint_ / axis.reduction_ratio_) : 0.0f;
            axis.send(msg);
        } else if (axis.torque_input_enabled_) {
            Set_Input_Torque_msg_t msg;
            msg.Input_Torque = axis.torque_setpoint_ / axis.reduction_ratio_;
            axis.send(msg);
        } else {
    ...
    switch (cmd) {
        case Get_Encoder_Estimates_msg_t::cmd_id: {
            if (Get_Encoder_Estimates_msg_t msg; try_decode(msg)) {
                pos_estimate_ = msg.Pos_Estimate * (2 * M_PI) / reduction_ratio_;
                vel_estimate_ = msg.Vel_Estimate * (2 * M_PI) / reduction_ratio_;
            }
        } break;
        case Get_Torques_msg_t::cmd_id: {
            if (Get_Torques_msg_t msg; try_decode(msg)) {
                torque_target_ = msg.Torque_Target / reduction_ratio_;
                torque_estimate_ = msg.Torque_Estimate * reduction_ratio_;
            }
        } break;
